...
#define MAGIC 253609357

#define BANDS "Bands_Fast"
#define BANDS_MA 0
#define BANDS_HI 1
#define BANDS_LO 2
...
//---- get indicator values
   double LowerBuffer = iCustom(NULL, 0, BANDS, BandsPeriod, MAPeriod, BandsShift, 
      BandsDeviations, BANDS_LO, 0);
   double UpperBuffer = iCustom(NULL, 0, BANDS, BandsPeriod, MAPeriod, BandsShift, 
      BandsDeviations, BANDS_HI, 0);
 ...
 //----
   ...
   if (InvestmentDirection < 2)
   {
      if (High[0] > UpperBuffer - B)
      {
         ...
         {
            type = OP_BUY;
            comment = "[TewiBoABCD] Opening type A, based on " + BANDS;
         }
         else if (InvestmentDirection == 1)
         {
            type = OP_SELL;
            comment = "[TewiBoABCD] Opening type B, based on " + BANDS;
         }
   ...
   else if (InvestmentDirection >= 2)
   {
      if (Low[0] < LowerBuffer - B)
      {
         ...
         {
            type = OP_BUY;
            comment = "[TewiBoABCD] Opening type C, based on " + BANDS;
         }
         else if (InvestmentDirection == 3)
         {
            type = OP_SELL;
            comment = "[TewiBoABCD] Opening type D, based on " + BANDS;
         }
