...
file_name = 'ROC_EURJPY_S_SearchBestK';
version = @ROC_S;
% file_name = 'ROC_EURJPY_S_SearchBestKv2';
% version = @ROC_Sv2;
rynek = 'EURJPY';
...
for j=k
    [profit, ~, ~, Calmar, iS] = version(C_learn,j,spread);
    if profit>bestReturn
        disp(['Wyniki uczenie --- Zysk: ' num2str(profit) ...
            ' --- Calmar: ' num2str(Calmar) ' dla: k=' num2str(j)...
            '. Otwartych pozycji krotkich: ' num2str(iS)]);
        bestReturn = profit;
        bestK = j;
    end
end

[profit, sumR, ROC_vec, Calmar,iS] = version(C_verrify,bestK,spread);
%%   WYNIKI
