...
file_name = 'ROC_EURJPY_L_SearchBestKD';
rynek = 'EURJPY';
...
for j=k
    for Duration = paramDuration
        [profit, ~, ~, Calmar, iL] = ROC_L_dur(C_learn,j,Duration,spread);
        if profit>bestReturn
            disp(['Wyniki uczenie --- Zysk: ' num2str(profit) ...
                ' --- Calmar: ' num2str(Calmar) ' dla: k=' num2str(j)...
                ', duration = ' num2str(Duration) '. Otwartych pozycji dlugich: ' num2str(iL)]);
            bestReturn = profit;
            bestK = j;
...
[profit, sumR, ROC_vec, Calmar,iL] = ROC_L_dur(C_verrify,bestK,bestDuration,spread);
%%   WYNIKI
disp(['Wyniki weryfikacja --- Zysk: ' num2str(profit) ...
    ' --- Calmar: ' num2str(Calmar) ' dla: k=' num2str(bestK)...
    '. Otwartych pozycji dlugich: ' num2str(iL)]);
