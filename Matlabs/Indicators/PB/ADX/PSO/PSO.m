function [ swarm, iterNum, best_l_op,best_zysk, best_calmar, lastPosLearnState, lastOpenPrice] = PSO( options,C,pocz,kon,spread, firstPosLearnState, lastOpenPrice)
...
while (iterNum < options.max_iter_num) && timeOutCounter < (options.best_position_timeout)
    ...
    for c=1:length(swarm.particles)
        ...
        [ zysk, Calmar, ~, LongShort, lastPosLearnState ] = Adx10fun( C, spread, pocz, ...
            kon, temp_p(1), temp_p(2), temp_p(3), temp_p(4), temp_p(5), firstPosLearnState, lastOpenPrice);
        %temp_p(6), temp_p(7) );     
