function [ swarm, iterNum, best_l_op,best_zysk ] = PSO( options,C,pocz,kon,spread)
...
while (iterNum < options.max_iter_num) && timeOutCounter < (options.best_position_timeout)
    ...
    for c=1:length(swarm.particles)
        ...
        swarm.particles(c).position([1,2,3]) =...
            round(swarm.particles(c).position([1,2,3]));
        %swarm.particles(c).position([5,6]) = SL([round(rand(1,2)*40+ones(1,2))]);
        temp_p = swarm.particles(c).position;
        [ zysk, Calmar, l_open] = StochasticOneParamSet( C, spread, pocz, kon, ...
