function [ zyl, Calmar, l_open] = tewiMiC( C, spread, pocz, kon, ...
    b, wstp, wstk, lkr, SL, TP, op, bvol, vwst, ll3,...
    bawe, bcawe)
...
for i=pocz:kon %40000:pocz+16000  %p?tla g??wna, ka?dy i-ty krok po pocz, to chwila bie??ca, nie dysponuje si? informacj? od i+1 wprz?d; dopisane 1000 ?wiec to swiece z przysz?osci
    lop(i)=lop(i-1);
    ...
    gwp(i)=max10(i);  %g?rna bariera wst?gi minimax
    dwp(i)=min10(i);
    ...
    %sprawdzeniee warunk?w zamkniecia poprzednio otwartych pozycji
    for j=1:ll  %(poprawi?)
        lll=lll+1;
    ...
    if i>=pocz
        l=l+1;
        C1(pocz+l,:)=F(l,:);  %dodanie nowej ?wiecy z macierzy F (przysz?ej, odcietej od przesz??o?ci); w tym miejscu powinien nastapi? import swiecy z platfoermy brokerskiej
