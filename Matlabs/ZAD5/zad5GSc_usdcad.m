...
for startingSection = 0:sectionCounter
                ...
                for vl = 1:length(VparamAVolThreshold)
                    ...
                    for vm = 1:length(VparamABuffer)
                        ...
                        for vn = 1:length(VparamASL)
                            ...
                            [ sumReturn,Calmar ] = Sc (C(bigPoint:bigPoint+paramASectionLearn+paramADuration,:),spread, paramADuration, paramAVolThreshold, paramABuffer, paramASL, maxes, volAverages,1, paramASectionLearn);
    ...
    [ sumReturn,Calmar ] = Sc (C(poczDanychTest:kon,:),spread,bestparamADuration, bestparamAVolThreshold, bestparamABuffer, bestparamASL, maxes, volAverages,max(bestparamALength,bestparamAVolLength),paramASectionTest);
