...
for personIndex = 1: length(osoby)
    ...
    for parNoise=1:length(noise)
        ...
        for i =1:scanPeriodCount
            ...
            op = functions{parNoise}(noise(parNoise)*C(9));
            bvol = functions{parNoise}(noise(parNoise)*C(10));
            vwst = functions{parNoise}(noise(parNoise)*C(11));
            ll3 = functions{parNoise}(noise(parNoise)*C(12));
